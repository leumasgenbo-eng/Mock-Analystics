...
const ReportCard: React.FC<ReportCardProps> = ({ student, stats, settings, onSettingChange, onStudentUpdate, classAverageAggregate, totalEnrolled, isFacilitator }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    const reportId = `report-capture-${student.id}`;
    const element = document.getElementById(reportId);
    if (!element) return setIsGenerating(false);
    
    const opt = { 
      margin: 0, 
      filename: `${student.name.replace(/\s+/g, '_')}_CAPI_REPORT.pdf`, 
      image: { type: 'jpeg', quality: 1.0 }, 
      html2canvas: { scale: 4, useCORS: true, logging: false, letterRendering: true, windowWidth: 794 }, 
    ...
    } catch (e) { console.error("CAPI Capture Fault:", e); } finally { setIsGenerating(false); }
  };

  return (
    <div className="flex flex-col items-center mb-24 group">
       {/* CAPI UI Branding Shard - Shown only in UI */}
       <div className="w-[210mm] no-print mb-8 px-12 animate-in fade-in duration-1000">
          <div className="flex items-center gap-4 mb-4 opacity-40">
             <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse"></div>
             <p className="text-[10px] font-black text-blue-900 uppercase tracking-[0.6em]">CAPI Identity Synchronization</p>
          </div>
          <div className="bg-slate-50 border border-slate-200 p-8 rounded-[3rem] flex flex-col items-center text-center space-y-3">
             ...
             <p className="text-[11px] font-bold text-gray-500 uppercase tracking-widest italic max-w-xl">
                <EditableField value={settings.schoolMotto || ""} onChange={(v) => onSettingChange('schoolMotto', v.toUpperCase())} className="text-center" />
             </p>
             <div className="text-[9px] font-black text-blue-800 uppercase tracking-[0.3em] flex gap-6">
                <span><EditableField value={settings.schoolAddress} onChange={(v) => onSettingChange('schoolAddress', v.toUpperCase())} /></span>
                <span className="opacity-20">|</span>
                <span><EditableField value={settings.schoolContact} onChange={(v) => onSettingChange('schoolContact', v)} /></span>
             </div>
          </div>
       </div>

       {/* OFFICIAL A4 SHARD - CAPTURE ZONE */}
       <div 
         id={`report-capture-${student.id}`} 
         className="bg-white w-[210mm] h-[297mm] shadow-[0_45px_100px_-15px_rgba(0,0,0,0.1)] print:shadow-none relative flex flex-col p-12 box-border font-sans overflow-hidden border border-gray-100 print:p-12 print:m-0"
       >
          {/* CAPI COMMAND BUTTONS */}
          <div data-html2canvas-ignore="true" className="absolute top-8 right-8 no-print flex gap-3 z-[100] opacity-0 group-hover:opacity-100 transition-all duration-500">
             <button onClick={() => window.print()} className="w-12 h-12 bg-blue-900 text-white rounded-2xl shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-all"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M6 9V2h12v7M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg></button>
             <button onClick={handleDownloadPDF} disabled={isGenerating} className={`w-12 h-12 rounded-2xl shadow-2xl flex items-center justify-center transition-all ${isGenerating ? 'bg-gray-400' : 'bg-red-600 text-white hover:scale-110'}`}>{isGenerating ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>}</button>
          </div>
          ...
          {/* IDENTITY MATRIX */}
          <div className="grid grid-cols-2 gap-10 mb-8 border-4 border-blue-900 p-8 rounded-[4rem] bg-blue-50/10 shrink-0">
          ...
          <div className="grid grid-cols-1 gap-5 mb-8 text-[13px] shrink-0">
             <div className="bg-slate-50 p-8 rounded-[3.5rem] border border-slate-200 shadow-inner relative">
                <span className="font-black text-blue-900 uppercase block text-[10px] mb-3 tracking-[0.4em]">Heuristic Performance Shard:</span>
                <p className="italic text-slate-800 leading-relaxed font-bold text-sm">"{dynamicAnalysis.performance}"</p>
             </div>
             <div className="grid grid-cols-2 gap-5">
               <div className="bg-blue-50/30 p-8 rounded-[3.5rem] border border-blue-100"><span className="font-black text-blue-900 uppercase block text-[9px] mb-2 tracking-[0.4em]">Character Profile:</span><p className="font-black text-blue-950 uppercase italic leading-tight text-sm">"{student.conductRemark || 'EXEMPLARY PROTOCOLS MAINTAINED.'}"</p></div>
               <div className="bg-indigo-50/30 p-8 rounded-[3.5rem] border border-indigo-100">
                  <span className="font-black text-indigo-900 uppercase block text-[9px] mb-2 tracking-[0.4em]">Strategic Guidance:</span>
          ...
          {/* NRT LEGEND */}
          <div className="mt-auto pt-8 border-t border-gray-100 shrink-0">
             <div className="bg-slate-950 text-white p-6 rounded-[3rem] flex justify-between items-center gap-12 border-4 border-white shadow-2xl">
                <div className="flex-1">
                   <h5 className="text-[10px] font-black uppercase tracking-[0.3em] mb-1 text-blue-400">NRT Analytical Logic (Distribution: {settings.useTDistribution ? 'T-DEGREES' : 'GAUSSIAN'})</h5>
                   <p className="text-[10px] leading-tight opacity-50 uppercase tracking-widest">Normalized grades relative to cohort mean. Lower aggregate implies higher instructional absorption.</p>
                </div>
                <div className="text-[11px] font-mono opacity-20 text-right uppercase tracking-[0.5em]">CAPI NODE<br/>{student.id.toString(16).toUpperCase()}</div>
             </div>
          </div>

          {/* SIGNATURE NODES */}
          <div className="flex justify-between items-end mt-12 pb-2 border-t-2 border-slate-100 pt-8 shrink-0">
