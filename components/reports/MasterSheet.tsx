...
const MasterSheet: React.FC<MasterSheetProps> = ({ students, stats, settings, onSettingChange, facilitators, isFacilitator }) => {
  ...
  const handleWhatsAppShare = () => {
    const classMean = Math.round(students.reduce((acc, s) => acc + s.totalScore, 0) / (students.length || 1));
    const summary = `*${settings.schoolName} - BROAD SHEET SUMMARY*\n` +
                    `*Series:* ${settings.activeMock}\n` +
                    `*Total Candidates:* ${students.length}\n` +
                    `*Cohort Mean Efficiency:* ${classMean}%\n\n` +
                    `_Generated via SS-Map Network Hub_`;
    window.open(`https://wa.me/?text=${encodeURIComponent(summary)}`, '_blank');
  };

  const handleDownloadNotepad = () => {
    let content = `${settings.schoolName} - MASTER BROAD SHEET\n`;
    content += `SERIES: ${settings.activeMock} | DATE: ${new Date().toLocaleDateString()}\n`;
    content += `================================================================\n\n`;
    content += `RANK | NAME                          | AGG | SCORE%\n`;
    content += `-----|-------------------------------|-----|-------\n`;
    
    students.forEach(s => {
      content += `${s.rank.toString().padEnd(4)} | ${s.name.substring(0, 28).padEnd(29)} | ${s.bestSixAggregate.toString().padEnd(3)} | ${Math.round(s.totalScore)}%\n`;
    });
    
    content += `\n================================================================\n`;
    content += `GENERATED BY SS-MAP NETWORK REGISTRY`;

    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `BroadSheet_${settings.activeMock.replace(/\s+/g, '_')}.txt`;
    link.click();
      ...
      <div className="no-print mb-8 space-y-4">
        <div className="flex flex-col xl:flex-row items-center justify-between bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-xl gap-6">
          <div className="flex items-center gap-6">
            <h3 className="text-xs font-black text-gray-500 uppercase tracking-widest border-r pr-6 border-gray-200">Broad Sheet Controller</h3>
          ...
          <div className="flex flex-wrap gap-3">
            <button 
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all active:scale-95 flex items-center gap-2"
            >
            ...
            <button 
              ...
              className="bg-slate-700 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all active:scale-95 flex items-center gap-2"
            >
            ...
            <button 
              ...
              className="bg-blue-950 hover:bg-black text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-xl transition-all active:scale-95 flex items-center gap-2"
            >
