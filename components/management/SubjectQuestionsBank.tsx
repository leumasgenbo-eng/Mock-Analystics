...
const SubjectQuestionsBank: React.FC<SubjectQuestionsBankProps> = ({ activeFacilitator, subjects }) => {
  ...
  // Dependent Taxonomy Logic
  const strands = useMemo(() => ['ALL', ...Array.from(new Set(masterBank.map(q => q.strand)))], [masterBank]);
  const subStrands = useMemo(() => {
    const subset = filterStrand === 'ALL' ? masterBank : masterBank.filter(q => q.strand === filterStrand);
    return ['ALL', ...Array.from(new Set(subset.map(q => q.subStrand)))];
  ...
  const indicators = useMemo(() => {
    ...
    return ['ALL', ...Array.from(new Set(subset.map(q => q.indicator)))];
  ...
  const handlePushToCloud = async () => {
    ...
    const payload = {
      ...
      questions: collectedQs, // Preservation of selection order
    ...
    try {
      ...
      alert("INSTRUCTIONAL SHARDS BROADCAST TO HUB CLOUD.");
      setCollectedQs([]);
    } catch (e) {
      ...
      <div className="bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden">
        ...
        <div className="relative flex flex-col md:flex-row justify-between items-center gap-8">
           <div className="space-y-1">
              <h2 className="text-2xl font-black uppercase tracking-tight">Curriculum Shard Bank</h2>
              <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Identify & Push Instructional Matrix</p>
           </div>
           ...
           <div className="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-xl space-y-6">
              ...
              <div className="space-y-4">
                 <div className="space-y-1">
                    ...
                    <select value={filterStrand} onChange={e=>setFilterStrand(e.target.value)} className="w-full bg-slate-50 border rounded-xl px-4 py-3 text-xs font-black uppercase">{strands.map(s => <option key={s} value={s}>{s}</option>)}</select>
                 </div>
                 <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-500 uppercase ml-2">Sub-Strand</label>
                    <select value={filterSubStrand} onChange={e=>setFilterSubStrand(e.target.value)} className="w-full bg-slate-50 border rounded-xl px-4 py-3 text-xs font-black uppercase">{subStrands.map(s => <option key={s} value={s}>{s}</option>)}</select>
                 </div>
                 <div className="space-y-1">
                    <label className="text-[8px] font-black text-slate-500 uppercase ml-2">Indicator</label>
                    <select value={filterIndicator} onChange={e=>setFilterIndicator(e.target.value)} className="w-full bg-slate-50 border rounded-xl px-4 py-3 text-xs font-black uppercase">{indicators.map(s => <option key={s} value={s}>{s}</option>)}</select>
                 </div>
              </div>
              ...
              <div className="pt-6 border-t border-gray-50">
                 ...
                 >
                   Broadcast {collectedQs.length} Shards
                 </button>
              </div>
           </div>
        </div>
           ...
           <table className="w-full text-left">
              <thead className="bg-slate-50 text-[8px] font-black text-slate-400 uppercase tracking-widest">
                 <tr>
                    ...
                    <th className="px-6 py-4">Question Shard</th>
                    ...
                    return (
                       <tr key={q.id} className="hover:bg-blue-50/30 transition-colors">
                          <td className="px-6 py-5">
                             <button onClick={() => toggleCollect(q)} className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center transition-all ${isCollected ? 'bg-blue-600 border-blue-600 text-white' : 'border-gray-200'}`}>
                                {isCollected && <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="4" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5"/></svg>}
                             </button>
                          </td>
                          <td className="px-6 py-5">
                             <span className="text-[10px] font-bold text-slate-600 uppercase italic">"{q.questionText}"</span>
                          </td>
                          <td className="px-6 py-5 text-right font-mono text-[9px] font-black text-blue-500">
                             {q.indicator}
                          </td>
                       </tr>
                    );
