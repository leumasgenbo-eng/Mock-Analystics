    ...
    <div className="space-y-12 animate-in fade-in duration-700 pb-20 font-sans">
      ...
      <section className="bg-slate-900 border border-white/5 p-8 rounded-[3rem] shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6">
         <div className="space-y-1">
            ...
            <p className="text-white font-black uppercase text-sm">Institutional Duty Management</p>
         </div>
         <div className="flex flex-wrap justify-center gap-3">
            ...
            <button onClick={() => setShowSchedule(!showSchedule)} className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all">
               {showSchedule ? 'Hide Master Time' : 'Generate Mock Time'}
            </button>
      ...
      {showSchedule && (
         <section className="bg-white p-10 rounded-[3.5rem] border-2 border-indigo-100 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center mb-10 border-b border-indigo-50 pb-6">
               ...
               <span className="text-[9px] font-black text-indigo-400 uppercase tracking-[0.4em]">Aggregated Timetable</span>
            </div>
        ...
        <section className="bg-slate-950 text-white p-10 rounded-[3rem] border border-white/5 shadow-2xl relative overflow-hidden">
           ...
           <div className="relative space-y-8">
              <div className="space-y-2">
                 <h2 className="text-3xl font-black uppercase tracking-tighter">{editingEmail ? 'Modulate Specialist' : 'Enroll Subject Specialist'}</h2>
                 <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.4em]">Authorized Hub Provisioning</p>
              </div>

              <form onSubmit={handleAddOrUpdateStaff} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                 ...
                 <button type="submit" disabled={isEnrolling} className="md:col-span-2 lg:col-span-3 bg-blue-600 hover:bg-blue-500 text-white py-5 rounded-2xl font-black text-[10px] uppercase shadow-lg transition-all active:scale-95">
                    {isEnrolling ? "SYNCING..." : editingEmail ? "Update Specialist Node" : "Activate Specialist Hub"}
                 </button>
              </form>
           </div>
        </section>
                  ...
                  <div className="flex items-center gap-6 flex-1">
                     ...
                     <div className="space-y-1.5">
                        <div className="flex items-center gap-3">
                           ...
                           <span className="px-3 py-0.5 rounded-lg text-[8px] font-black uppercase bg-emerald-50 text-emerald-600">Subject Specialist</span>
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-[9px] font-bold text-gray-400 uppercase tracking-widest">
                           ...
                           <span className="font-mono text-[8px] text-indigo-600">Enrolled Institutional ID: {f.enrolledId || '---'}</span>
                        </div>
                     </div>
                  </div>
                  
                  <div className="flex flex-wrap justify-center gap-3">
                     <button onClick={() => handleEditSetup(f)} className="bg-gray-50 text-slate-600 px-5 py-2.5 rounded-xl font-black text-[9px] uppercase border border-gray-200 hover:bg-white transition-all">Edit Particulars</button>
                     <button onClick={() => setExpandedStaff(isExpanded ? null : f.email)} className={`px-8 py-2.5 rounded-xl font-black text-[9px] uppercase transition-all ${isExpanded ? 'bg-blue-900 text-white shadow-lg' : 'bg-slate-900 text-white'}`}>
                        {isExpanded ? 'Hide' : 'Audit Shard'}
                     </button>
                  </div>
               </div>
                        ...
                        <div className="lg:col-span-9 bg-white p-10 rounded-[3rem] border border-gray-100 space-y-8 shadow-sm">
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                              {Array.from({ length: 9 }).map((_, idx) => {
                                 ...
                                 return (
                                    <div key={idx} className="bg-slate-50 p-6 rounded-[2.5rem] border border-gray-100 space-y-4 group/slot hover:border-indigo-200 transition-all shadow-sm">
