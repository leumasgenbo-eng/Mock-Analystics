...
const PupilSBAPortal: React.FC<PupilSBAPortalProps> = ({ students, setStudents, settings, subjects, onSave }) => {
  ...
  const handleAddOrUpdateStudent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setStudents(nextStudents);
        onSave({ students: nextStudents });
        alert(`ENROLLED: ID ${studentId} | PIN ${accessPin}`);
      }
  ...
  const handleDownloadTemplate = () => {
    const csvContent = "Name,Gender(M/F),GuardianName,ParentContact,ParentEmail\nJohn Doe,M,Jane Doe,0240000000,john@example.com";
    const blob = new Blob([csvContent], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", "Pupil_Registry_Template.csv");
    link.click();
  };
  ...
  const handleExportRegistry = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", `Registry_Export_${settings.schoolNumber}.csv`);
    link.click();
  };
  ...
  const handleBulkUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      setStudents(nextStudents);
      onSave({ students: nextStudents });
      alert(`BULK HANDSHAKE COMPLETE: ${dataLines.length} candidates added to registry.`);
    ...
    <div className="space-y-12 animate-in fade-in duration-500 pb-20 font-sans">
      ...
      <section className="bg-slate-950 border border-white/5 p-8 rounded-[3rem] shadow-2xl flex flex-col md:flex-row justify-between items-center gap-6">
         <div className="space-y-1">
            <h4 className="text-[10px] font-black text-blue-400 uppercase tracking-[0.4em]">Matrix Control</h4>
            <p className="text-white font-black uppercase text-sm">Institutional Identity & SBA</p>
         </div>
         <div className="flex flex-wrap justify-center gap-3">
            <button onClick={handleDownloadTemplate} className="bg-white/5 hover:bg-white/10 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase border border-white/10 transition-all">Download Template</button>
            <button onClick={handleExportRegistry} className="bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase border border-white/10 transition-all">Extract Shard</button>
            <button onClick={() => fileInputRef.current?.click()} className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all">Bulk Upload</button>
      ...
      <section className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-2xl relative overflow-hidden">
        <div className="flex items-center gap-4 mb-8">
           ...
           <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">{editingId ? 'Modulate Shard' : 'Direct Enrolment'}</h3>
        </div>
        ...
        <form onSubmit={handleAddOrUpdateStudent} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-1 md:col-span-2">
             ...
             <input type="email" value={formData.parentEmail} onChange={e=>setFormData({...formData, parentEmail: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-black outline-none" placeholder="REPORTS@FAMILY.COM" />
          </div>
          ...
          <div className="lg:col-span-3 pt-4 flex gap-4">
             <button type="submit" disabled={isEnrolling} className={`flex-1 ${editingId ? 'bg-indigo-600' : 'bg-blue-900'} text-white py-5 rounded-3xl font-black text-[11px] uppercase tracking-[0.3em] shadow-2xl active:scale-95 transition-all`}>
                {isEnrolling ? "Syncing..." : editingId ? "Update Identity" : "Activate Shard"}
             </button>
      ...
      <div className="space-y-6">
         <div className="relative">
            <input 
              ...
              className="w-full bg-white border border-gray-100 rounded-3xl px-14 py-5 text-sm font-bold shadow-xl outline-none focus:ring-8 focus:ring-blue-500/5 transition-all uppercase" 
            />
         ...
         <div className="grid grid-cols-1 gap-4">
            {filteredStudents.map(s => {
               ...
               return (
                 <div key={s.id} className="bg-white rounded-[2.5rem] border border-gray-100 shadow-lg overflow-hidden group hover:border-blue-200 transition-all">
                    <div className="p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6">
                       <div className="flex items-center gap-6 flex-1">
                          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl border-4 border-white shadow-md ${s.gender === 'F' ? 'bg-pink-100 text-pink-600' : 'bg-blue-100 text-blue-600'}`}>{s.name.charAt(0)}</div>
                          <div className="space-y-1">
                       ...
                       <div className="flex gap-2">
                          <button onClick={() => toggleLedger(s.id)} className={`px-5 py-2.5 rounded-xl font-black text-[9px] uppercase transition-all flex items-center gap-2 ${isExpanded ? 'bg-indigo-900 text-white shadow-lg' : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'}`}>
                             {isExpanded ? 'Hide Ledger' : 'SBA Ledger'}
                             <svg className={`transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="6 9 12 15 18 9"/></svg>
                          </button>
                    ...
                    {/* SBA Score Accordion */}
                    {isExpanded && (
                       <div className="bg-slate-50 p-8 md:p-10 border-t border-gray-100 animate-in slide-in-from-top-4 duration-500">
                          <div className="flex justify-between items-center mb-8">
                             <div className="space-y-1">
                                <h5 className="text-sm font-black text-indigo-950 uppercase tracking-widest">SBA Assessment Grid</h5>
                                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.3em]">Cycle: {settings.activeMock} • Weight: {settings.sbaConfig.sbaWeight}%</p>
                             </div>
                          </div>

                          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                             {subjects.map(sub => (
                                <div key={sub} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-2 group/sba hover:border-indigo-300 transition-all">
                                   ...
                                   <input 
                                      ...
                                      placeholder="0"
                                      className="w-full bg-slate-50 border-2 border-transparent rounded-xl py-2.5 text-center font-black text-blue-900 text-lg outline-none focus:bg-white focus:border-indigo-500 transition-all"
                                   />
                                </div>
                             ))}
                          </div>
                          ...
                          <div className="mt-8 flex justify-end">
                             <button onClick={() => { onSave(); setExpandedId(null); alert("SBA SHARDS COMMITTED."); }} className="bg-indigo-900 text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase shadow-xl hover:bg-black transition-all">Finalize Shards</button>
                          </div>
                       </div>
                    )}
                 </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-[3rem] p-10 flex items-start gap-8 shadow-inner">
         ...
         <div className="space-y-2">
            <h4 className="text-xs font-black text-blue-900 uppercase">Registry Protocol</h4>
            <p className="text-[10px] text-blue-700 font-bold leading-relaxed uppercase tracking-widest">
               Enrolled identities are shared across the network. PINs allow pupils to recall their performance shards from any device. Ensure CSV formatting matches the provided template for bulk ingestion.
            </p>
         </div>
      </div>
    </div>
